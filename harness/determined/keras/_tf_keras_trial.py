...
class TFKerasTrialController(det.LoopTrialController):
    ...
    def _convertBackFromTensor(self, possibleTensor):
    ...
    def _post_train_batch_end(self, num_inputs: int, logs: Dict) -> None:
        ...
        self.train_workload_metrics.append(
            {k: self._convertBackFromTensor(v) for k, v in logs.items() if k not in {"batch", "size"}}
        )
        ...
        if self.hvd_config.use:
            ...
            num_inputs = self._convertBackFromTensor(num_inputs)
